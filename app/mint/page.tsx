...
// Minimal ERC-721A style mint ABI (sesuaikan dgn kontrak kamu).
const MINT_ABI = [
  {
    type: "function",
    name: "mint",
...
export default function MintPage() {
  ...
  // Progres mint ilustratif untuk tampilan.
  const minted = 1287;
  const pct = useMemo(
  ...
  function handleMint() {
    ...
    writeContract({
      ...
      functionName: "mint",
      args: [BigInt(qty)],
      value: 0n, // free mint; ganti kalau berbayar
    });
  }
        ...
        <div className="grid gap-10 lg:grid-cols-2 items-start">
          ...
          <div className="panel p-6">
            <div className="aspect-square rounded-xl border border-stonk-line grid-bg relative overflow-hidden grid place-items-center">
              ...
              <div className="absolute top-3 left-3 chip px-2 py-1 text-xs text-stonk-green">
                Robinhood Chain
              </div>
            </div>
            <div className="mt-5">
          ...
          <div>
            ...
            <p className="mt-3 text-stonk-muted">
              Klaim broker NFT-mu di {robinhoodChain.name}. Tiap broker adalah tiket masuk
              ke ekosistem dan reward yang ditenagai {site.ticker} di Pons.
            </p>
            ...
            <div className="mt-6 panel p-5 space-y-5">
              ...
              {!isConnected ? (
                ...
                >
                  {wrongChain
                    ...
                    ? "Check wallet…"
                    : isDemo
                    ? "Mint (demo — kontrak belum di-set)"
                    : `Mint ${qty}`}
                </button>
              )}
              ...
              {isDemo && (
                <p className="text-xs text-stonk-gold">
                  ⚠ Mode demo: isi <code>nft.contractAddress</code> di{" "}
                  <code>lib/config.ts</code> untuk mengaktifkan mint on-chain.
                </p>
              )}
              {isSuccess && (
                <p className="text-xs text-stonk-green">
                  ✅ Mint berhasil! Cek wallet-mu.
                </p>
              )}
              {error && (
